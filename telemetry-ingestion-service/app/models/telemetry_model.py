...
class CamaVermicompostaje(Base):
    ...
    nodos = relationship("NodoSensor", back_populates="cama", cascade="all,delete")
...
class NodoSensor(Base):
    ...
    cama_id = Column(Integer, ForeignKey("cama_vermicompostaje.cama_id", ondelete="CASCADE"), nullable=False, index=True)
    codigo_nodo = Column(String(120), nullable=False, unique=True, index=True)
    ...
    lecturas = relationship("Lectura", back_populates="nodo", cascade="all,delete")
...
class Lectura(Base):
    ...
    nodo_id = Column(Integer, ForeignKey("nodo_sensor.nodo_id", ondelete="CASCADE"), nullable=False, index=True)
...
class LecturaInvalida(Base):
    ...
    nodo_id = Column(Integer, ForeignKey("nodo_sensor.nodo_id", ondelete="SET NULL"), nullable=True, index=True)
    tipo_variable_id = Column(Integer, ForeignKey("tipo_variable.tipo_variable_id", ondelete="SET NULL"), nullable=True, index=True)
